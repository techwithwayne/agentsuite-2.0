...
TechWithWayne Digital Solutions
        ...
        try:
            ...
            return {
                "message": f"Shirley: Thanks {name}! Your diagnostic report is on its way to {email}. Keep an eye on your inbox (and spam folder) — it should arrive within a few minutes.",
                "success": True
            }
...
NEVER use "stage_will_be_set_by_system"."""
        ...
        try:
            ...
            # 🔧 If the user just agreed (offered_report -> hybrid_closing), ensure the text is a form prompt
            if stage == "offered_report" and forced_stage == "hybrid_closing":
                # Replace whatever the model said (likely another offer) with a brief form instruction
                assistant_msg["response"] = (
                    "Perfect! Please fill out the form that just appeared so I can send your report."
                )
                if assistant_msg.get("category") is None:
                    assistant_msg["category"] = category
